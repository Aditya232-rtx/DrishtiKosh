...
class ForestWorld {
    ...
    /**
     * Set rendering mode for forest (vibrant, dull, or normal)
     * @param {string} mode - 'vibrant', 'dull', or 'normal'
     */
    setRenderMode(mode) {
        const canvas = this.renderer.domElement;
        
        // Remove existing mode classes
        canvas.classList.remove('forest-vibrant', 'forest-dull');
        
        // Apply new mode
        if (mode === 'vibrant') {
            canvas.classList.add('forest-vibrant');
        } else if (mode === 'dull') {
            canvas.classList.add('forest-dull');
        }
    }
}
