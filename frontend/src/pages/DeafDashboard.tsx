...
const DeafDashboard = () => {
    ...
    useEffect(() => {
        const fetchUserProfile = async () => {
            try {
                ...
                setUserType(user.learning_preference || "deaf");
    ...
    const handleNewSession = () => {
        ...
        if (urlMatch) {
            // ANY URL found → Route to DeafVideoMode
            const extractedUrl = urlMatch[0];
            const encodedUrl = encodeURIComponent(extractedUrl);
            navigate(`/deaf-video?url=${encodedUrl}`);
        } else {
            // No URL (plain text topic) → Route to DeafLearnMode
            const encodedTopic = encodeURIComponent(trimmedMessage);
            if (uploadContext) {
                navigate(`/deaf-learn?pending_context=true&topic=${encodedTopic}`);
            } else {
                navigate(`/deaf-learn?topic=${encodedTopic}`);
            }
        }
    };
                ...
                <div className="flex-1 overflow-y-auto px-2 custom-scrollbar">
                    ...
                    <div className="space-y-1 pb-4">
                        {(isHistoryExpanded ? chatHistory : chatHistory.slice(0, 5)).map((chat) => (
                            <button
                                key={chat.id}
                                onClick={() => navigate(`/deaf-learn?sessionId=${chat.id}`)}
                ...
                <div className="max-w-4xl mx-auto">
                    ...
                    <div className="flex items-center justify-between mb-8">
                        <div className="animate-fade-in">
                            ...
                            <p className="text-muted-foreground">
                                Continue where you left off or start a new visual learning session
                            </p>
                        </div>
                    ...
                    <div className="bg-gradient-to-r from-primary/10 via-primary/5 to-transparent rounded-2xl p-8 border border-primary/20">
                        ...
                        <p className="text-muted-foreground mb-6">
                            Start a new visual learning session by asking a question or entering a topic
                        </p>
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                        <div className="flex items-center gap-3 p-3 bg-background rounded-lg border border-border">
                            ...
                            <div>
                                <p className="text-sm font-medium">Visual Learner</p>
                                <p className="text-xs text-muted-foreground">Completed 5 visual lessons</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-3 p-3 bg-background rounded-lg border border-border">
                            ...
                            <div>
                                <p className="text-sm font-medium">Image Master</p>
                                <p className="text-xs text-muted-foreground">Analyzed 10 images</p>
                            </div>
                        </div>
                    </div>
                </div>
